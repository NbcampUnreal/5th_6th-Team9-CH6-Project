...
//////////////////////////////////////////////////////////////////////////
// AStillBoundCharacter

AStillBoundCharacter::AStillBoundCharacter()
{
...
void AStillBoundCharacter::BeginPlay()
{
	Super::BeginPlay();
	...
	if (AIController)
	{
		...
		AIController->ReceiveMoveCompleted.RemoveDynamic(this, &AStillBoundCharacter::OnMoveCompleted);
		// 이동 완료 이벤트 델리게이트 바인딩
		AIController->ReceiveMoveCompleted.AddDynamic(this, &AStillBoundCharacter::OnMoveCompleted);
...
void AStillBoundCharacter::NotifyControllerChanged()
{
...
void AStillBoundCharacter::SetupPlayerInputComponent(UInputComponent* PlayerInputComponent)
{
	...
	if (UEnhancedInputComponent* EnhancedInputComponent = Cast<UEnhancedInputComponent>(PlayerInputComponent)) {
		...
		// Moving
		EnhancedInputComponent->BindAction(MoveAction, ETriggerEvent::Triggered, this, &AStillBoundCharacter::Move);

		// Looking
		EnhancedInputComponent->BindAction(LookAction, ETriggerEvent::Triggered, this, &AStillBoundCharacter::Look);
	}
	else
	{
...
void AStillBoundCharacter::Move(const FInputActionValue& Value)
{
	// input is a Vector2D
...
void AStillBoundCharacter::Look(const FInputActionValue& Value)
{
	// input is a Vector2D
...
void AStillBoundCharacter::FindTargetPoints()
{
...
void AStillBoundCharacter::StartMoving()
{
	// 타겟 포인트를 찾고 이동 시작
	FindTargetPoints();
	MoveToTarget();
...
void AStillBoundCharacter::MoveToTarget()
{
	if (!AIController)
	{
...
void AStillBoundCharacter::OnMoveCompleted(FAIRequestID RequestID, EPathFollowingResult::Type Result)
{
	bIsMoving = false;
	...
	if (Result == EPathFollowingResult::Success)
	{
		...
		GetWorldTimerManager().SetTimer(TimerHandle, this, &AStillBoundCharacter::MoveToTarget, 0.5f, false);
	}
	else
	{
		// 이동 실패
		...
		// 실패 시에도 다시 시도
		FTimerHandle TimerHandle;
		GetWorldTimerManager().SetTimer(TimerHandle, this, &AStillBoundCharacter::MoveToTarget, 1.0f, false);
	}
}